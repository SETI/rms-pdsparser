...
# The docstrings name these classes as a type annotation would after
# `from pathlib import Path` and `from filecache import FCPath`. Point those short names at
# their documented targets; intersphinx then resolves them.
_TYPE_TARGETS = {
    'Path': 'pathlib.Path',
...
def _resolve_short_type_names(app, env, node, contnode):
    target = _TYPE_TARGETS.get(node.get('reftarget'))
    if node.get('refdomain') != 'py' or target is None:
        return None
...
def setup(app):
    app.connect('missing-reference', _resolve_short_type_names)
